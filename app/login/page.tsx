...
export default function LoginPortal() {
    ...
    const handleGoogleLogin = async () => {
        ...
        const timeout = setTimeout(() => {
            setIsLoading(false);
            console.log("Google Login failed! .env.local mein GOOGLE_CLIENT_ID missing hai.");
        }, 5000);
        ...
        try {
            ...
            clearTimeout(timeout);
            console.log("System Error. Check Console.");
            setIsLoading(false);
        }
    };
 ...
 const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!name || !phone || !password) return console.log("Please fill all details.");
        ...
        try {
            ...
            if (res.ok && data.success) {
                ...
                console.log(data.error || "Verification failed.");
            }
        } catch (error) {
            console.error("Register Error:", error);
            console.log("Database Error. Backend check karein.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleManualLogin = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!phone || !password) return console.log("Please enter details.");
        ...
        try {
            ...
            if (res?.error) {
                console.log(res.error || "Invalid Details");
                setIsLoading(false);
            } else {
                router.push('/account');
            }
            ...
            console.log("Login System Offline.");
            setIsLoading(false);
        }
    };
