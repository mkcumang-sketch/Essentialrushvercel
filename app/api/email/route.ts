...
const ALLOWED_EMAIL_ROLES = new Set(["SUPER_ADMIN", "ADMIN", "STAFF", "AGENT"]);
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!session?.user || !ALLOWED_EMAIL_ROLES.has(role)) {
    ...
    if (!resendApiKey) {
      return NextResponse.json({ success: false, error: "Email is not configured on the server" }, { status: 500 });
    }

    const resend = new Resend(resendApiKey);
    const safeCustomerName = escapeHtml(customerName || "Valued Customer");
    const safeOrderId = escapeHtml(orderId || "10045X").slice(-6).toUpperCase();
    const safeAmount = Number(amountValue).toLocaleString("en-IN");
    ...
    const data = await resend.emails.send({
      ...
      html: `
        <div style="font-family: Helvetica, sans-serif; background-color: #FAFAFA; padding: 40px 20px; color: #050505;">
          <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; padding: 40px; border-radius: 16px; border: 1px solid #eaeaea;">
            ...
            <h2 style="font-size: 20px; margin-bottom: 20px;">Hello ${safeCustomerName},</h2>
            ...
            <div style="background-color: #FAFAFA; padding: 20px; border-radius: 12px; margin: 30px 0; border: 1px solid #eee;">
              ...
              <p style="margin: 0 0 5px 0; font-size: 16px;"><strong>Order ID:</strong> #${safeOrderId}</p>
              <p style="margin: 0; font-size: 16px;"><strong>Total Amount:</strong> ₹${safeAmount}</p>
            </div>
