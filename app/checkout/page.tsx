  ...
  const processPayment = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (res.ok && data.success) {
            ...
            setErrorMessage("Order failed: " + (data.error || "Unknown error"));
            setShowErrorDialog(true);
        }
    } catch (error) {
                  ...
                  <div className="space-y-6 mb-8 max-h-[300px] overflow-y-auto pr-2">
                    {items.map((item) => (
                      <div key={item._id} className="flex gap-4">
                        <div className="w-20 h-20 bg-gray-50 rounded-xl p-2 border border-gray-100 shrink-0"><img src={item.imageUrl || '/placeholder.png'} className="w-full h-full object-contain" /></div>
