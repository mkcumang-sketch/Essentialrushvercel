...
import { ShoppingBag, Star, ShieldCheck, ArrowLeft, Play, Image as ImageIcon } from 'lucide-react';
...
export default function ProductClientPage({ initialProduct }: ProductClientPageProps) {
    ...
    // 🔧 2. HYDRATION-SAFE INITIALIZATION EFFECT
    useEffect(() => {
        // Mark component as mounted on the client first
        setIsMounted(true);
        
        // Now it's 100% safe to access localStorage without Next.js errors
        setCart(JSON.parse(localStorage.getItem('luxury_cart') || '[]'));
        
        // Initialize Media (Images & Videos)
        ...
        const fetchReviews = async () => {
            try {
                ...
                let pubReviews = (revRes.data || []).filter((r: any) => 
                    (r.product === initialProduct._id || r.product === 'GLOBAL') && r.visibility === 'public'
                );
                
                const localReviews = JSON.parse(localStorage.getItem('my_ghost_reviews') || '[]').filter((r: any) => 
                ...
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col justify-center">
                    ...
                    <div className="bg-white p-6 rounded-[24px] border border-gray-200 mb-10">
                        ...
                        <div className="grid grid-cols-2 gap-y-4 text-sm">
                            {(initialProduct.amazonDetails || []).map((detail: { key: string; value: string }, idx: number) => (
