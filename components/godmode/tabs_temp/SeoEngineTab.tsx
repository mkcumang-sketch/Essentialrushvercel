...
export default function SeoEngineTab() {
  ...
  const handleLaunchTool = async (toolId: string, title: string) => {
    ...
    try {
      if (toolId === "meta-synthesizer") {
        const res = await fetch("/api/seo/synthesize-all", { method: "POST" });
        const data = await res.json();
        if (res.ok && data.success) {
          setToolResult({
            type: "success",
            message: `AI Meta Synthesizer completed! Automatically generated & indexed meta titles and descriptions for ${data.count || "all"} timepieces.`,
          });
          fetchSeoStats(true);
        } else {
          setToolResult({
      ...
      } else if (toolId === "alt-generator") {
        const res = await fetch("/api/seo/synthesize-all", { method: "POST" });
        const data = await res.json();
        if (res.ok) {
          setToolResult({
            type: "success",
            message: "ALT tags generated! Accessibility tags mapped to catalog media.",
          });
          fetchSeoStats(true);
        } else {
          setToolResult({ type: "error", message: data.error || "Failed to update ALT tags." });
        }
      } else if (toolId === "keywords") {
        setToolResult({
          type: "info",
          message: "High-Volume Target Keywords: Luxury Timepieces, Swiss Chronograph, Automatic Movement, Horology Vault, Certified Watches.",
        });
      ...
      setToolResult({
        type: "error",
        message: "Network error executing utility. Please ensure your backend endpoints are online.",
      });
    } finally {
      ...
      <div className="w-full min-h-[500px] flex items-center justify-center">
        ...
        >
          ...
          <button
            ...
            className="mt-7 inline-flex items-center gap-2 px-6 py-3 bg-white text-black rounded-xl text-xs font-black uppercase tracking-widest hover:bg-gray-200 transition-all shadow-lg cursor-pointer"
          >
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      className="w-full space-y-8 pb-24 text-white font-sans"
    >
      {/* HEADER SECTION */}
        ...
        <div className="relative p-6 md:p-8 flex flex-col xl:flex-row justify-between gap-7">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              disabled={refreshing}
              className="inline-flex items-center gap-2 px-5 py-3 bg-[#D4AF37] text-black rounded-xl text-xs font-black uppercase tracking-widest hover:bg-white transition-all disabled:opacity-50 shadow-[0_0_20px_rgba(212,175,55,0.15)] cursor-pointer"
            >
        ...
        <div className="bg-gradient-to-b from-[#D4AF37]/10 to-black/40 border border-[#D4AF37]/20 rounded-[2rem] p-6 md:p-8 relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col h-full justify-between">
            ...
            <button
              ...
              className="mt-6 w-full py-3.5 rounded-xl bg-white text-black text-[10px] font-black uppercase tracking-widest hover:bg-[#D4AF37] transition-colors flex items-center justify-center gap-2 shadow-lg cursor-pointer"
            >
        ...
        {navigation.map((item) => {
          ...
          return (
            <button
              ...
              className={`relative flex items-center gap-2 px-5 py-3 rounded-xl whitespace-nowrap text-[10px] font-bold uppercase tracking-widest transition-colors cursor-pointer ${
          ...
          <motion.div key="tools" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              <ToolCard
                ...
                description="Generate SEO-friendly meta titles and descriptions from product data."
                onLaunch={() => handleLaunchTool("meta-synthesizer", "AI Meta Synthesizer")}
              />
              <ToolCard
                icon={ImageIcon}
                title="ALT Text Generator"
                description="Create descriptive, accessibility-friendly image ALT text."
                onLaunch={() => handleLaunchTool("alt-generator", "ALT Text Generator")}
              />
              <ToolCard
                icon={Search}
                title="Keyword Analyzer"
                description="Analyze focus keywords and content relevance."
                onLaunch={() => handleLaunchTool("keywords", "Keyword Analyzer")}
              />
              <ToolCard
                icon={FileText}
                title="Sitemap Inspector"
                description="Check sitemap availability and indexing configuration."
                onLaunch={() => handleLaunchTool("sitemap", "Sitemap Inspector")}
              />
              <ToolCard
                icon={ShieldCheck}
                title="Robots.txt Check"
                description="Validate crawler access and blocked routes."
                onLaunch={() => handleLaunchTool("robots", "Robots.txt Check")}
              />
              <ToolCard
                icon={Globe}
                title="Canonical Inspector"
                description="Detect missing or inconsistent canonical URLs."
      ...
      {/* REAL UTILITY EXECUTION MODAL */}
      {activeToolModal && (
        <div className="fixed inset-0 z-[1000] bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <motion.div 
            initial={{ scale: 0.95, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            className="bg-[#12161A] border border-white/15 p-8 rounded-3xl max-w-lg w-full relative shadow-2xl"
          >
            <button
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-lg font-bold text-white font-serif">{modalTitle}</h3>
            </div>
            ...
            {toolProcessing ? (
              <div className="py-12 flex flex-col items-center justify-center gap-4 text-center">
                ...
                <p className="text-xs font-bold uppercase tracking-widest text-gray-300">
                  Executing Catalog Task...
                </p>
                <p className="text-[11px] text-gray-500">Communicating with database and search engines.</p>
              </div>
            ) : (
              <div className="space-y-6">
                ...
                <div className="flex justify-end gap-3 pt-2">
                  <button
                    type="button"
                    onClick={() => setActiveToolModal(null)}
                    className="w-full py-3.5 bg-[#D4AF37] hover:bg-white text-black font-black uppercase text-xs tracking-widest rounded-xl transition-all cursor-pointer"
                  >
                    Done
                  </button>
                </div>
...
function ActionCard({ icon: Icon, title, description, onClick }: { icon: LucideIcon; title: string; description: string; onClick: () => void }) {
  return (
    <motion.button type="button" whileHover={{ y: -4 }} whileTap={{ scale: 0.98 }} onClick={onClick} className="text-left bg-[#0a0a0a] border border-white/10 rounded-2xl p-6 hover:border-[#D4AF37]/40 hover:bg-white/[0.02] transition-all group shadow-lg relative overflow-hidden cursor-pointer">
...
/* =========================================================
   TOOL CARD (CONNECTED TO REAL LAUNCH HANDLER)
========================================================= */
  ...
  return (
    <motion.div whileHover={{ y: -4 }} className="bg-[#0a0a0a] border border-white/10 rounded-[2rem] p-7 shadow-lg flex flex-col justify-between group">
      ...
      <motion.button
        ...
        type="button"
        onClick={onLaunch}
        className="mt-8 w-full py-3.5 rounded-xl bg-white/5 hover:bg-[#D4AF37] hover:text-black border border-white/10 text-white text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 cursor-pointer shadow-md"
      >
...
function QuickHeaderButton({ icon: Icon, label, onClick }: { icon: LucideIcon; label: string; onClick: () => void }) {
  return (
    <motion.button
      ...
      type="button"
      onClick={onClick}
      className="inline-flex items-center gap-2 px-5 py-3 bg-white/5 border border-white/10 text-white rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-white hover:text-black transition-all shadow-sm cursor-pointer"
    >
      <Icon size={16} />
      {label}
    </motion.button>
