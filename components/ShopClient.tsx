...
export default function ShopClient({ initialProducts = [] }: { initialProducts: Product[] }) {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    if (sortBy === "PRICE_LOW") {
      result.sort((a, b) => Number(a.offerPrice || a.price) - Number(b.offerPrice || b.price));
    } else if (sortBy === "PRICE_HIGH") {
      result.sort((a, b) => Number(b.offerPrice || b.price) - Number(a.offerPrice || a.price));
    } else {
  ...
  const addToCart = (product: Product, e: React.MouseEvent) => {
    ...
    notify(`${product.name || product.title || "Timepiece"} added to Vault`);
  };
        ...
        <aside className="lg:w-[280px] flex-shrink-0 lg:sticky lg:top-32 self-start">
          <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] mb-6">
            ...
            <h4 className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-6 flex items-center gap-2">
              <SlidersHorizontal size={14}/> Top Brands
            </h4>
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 lg:gap-8">
            {filteredAndSortedProducts.map((watch) => {
              const displayTitle = watch.name || watch.title || "Masterpiece Timepiece";
              const watchImage = (watch.images && watch.images[0]) || watch.imageUrl || "";

              return (
                <Link 
                  href={`/product/${watch.slug || watch._id}`} 
                  key={watch._id} 
                  className="group bg-white p-6 rounded-[2.5rem] border border-gray-100 hover:border-gray-300 shadow-[0_8px_30px_rgb(0,0,0,0.02)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] transition-all duration-500 flex flex-col h-full relative"
                >
                  ...
                  <div className="flex-1 flex flex-col justify-between px-2">
                    <div>
                      ...
                      <h4 className="text-xl font-serif text-[#050505] leading-tight mb-4 tracking-tighter group-hover:text-[#D4AF37] transition-colors line-clamp-2 font-black">
                        {displayTitle}
                      </h4>
                    </div>
                    ...
                    <div className="flex justify-between items-end mt-auto pt-6 border-t border-gray-100">
                      <div>
                        ...
                        <p className="text-2xl text-black font-black font-serif">
                      ...
                      <motion.button 
                        ...
                        aria-label={`Add ${displayTitle} to vault`}
